...
try:
    ...
    print("qgrid and qgridnext are considered deprecated. Please stop using it.")
...
#            dict_size = dict(zip(result_df.columns.tolist(), maxColumnLenghts))
    ...
    def itablesDisplay(self, result_df, mycnt):
    ...
    def qgridDisplay(self, result_df, mycnt):
        ...
        if self.opts['display_qg_autofit_cols'][0] == True:
            ...
            for col in range(len(result_df.columns)):
                ...
                lengths = result_df.iloc[:,col].apply(safe_len)
...
# This can now be more easily extended with different display types
    def displayDF(self, result_df, instance=None, qtime=None):
        ...
        if mycnt == 0:
            ...
            elif display_type == "itables":
                self.itablesDisplay(result_df, mycnt)
            else:
        ...
        # if it exists in the user namespace
            ...
            if not dot_df:
                ...
                try:
                    ...
                    pass
            if sq_df or dot_df:
                thisdf = self.ipy.user_ns[myvar]
                trun = tvar.replace(myvar, "thisdf")
                try:
                    tmpdf = eval(trun)
